...
import { Search, Filter, MoreHorizontal, Edit, Trash2, Eye, Lock, Unlock } from 'lucide-react';

const DataTable = ({ 
  data, 
  columns, 
  onEdit, 
  onDelete,     // giữ tên để tương thích, thực tế bạn đang toggle lock/unlock
  onView,
  searchable = true,
  filterable = true,
  pagination = true,
  // 👇 mới thêm
  getDeleteLabel,          // (row) => string
  getDeleteIcon            // (row) => IconComponent
}) => {
        ...
        <table className="table w-full">
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {paginatedData.map((item, index) => {
              const label = getDeleteLabel ? getDeleteLabel(item) : 'Delete';
              const Icon = getDeleteIcon ? getDeleteIcon(item) : Trash2;
